...
c   ./sumlistnkginfo < sumlistnkginfo.i002345 > sumlistnkginfo.out002345
c-----------------------------------------------------------------------
      ...
      do  is=1,98765
         ...
         if ( index(czeile,'PRIMARY ENERGY') .gt. 0 ) then
            if ( index(czeile,'STCKIN:') .gt. 0 ) then
               write(cfmtexp(3:4),'(i2)') 15
               read(czeile(57:71),cfmtexp) energy
               lflagegy = 1
            elseif ( index(czeile,'FIXED') .gt. 0 ) then
               jfix = index(czeile,'FIXED') + 8
               jgev = index(czeile,'GEV') - 1
               write(cfmtexp(3:4),'(i2)') jgev-jfix+1
               read(czeile(jfix:jgev),cfmtexp) energy
               lflagegy = 1
            elseif ( index(czeile,' ENERGY = ') .gt. 0 ) then
               write(cfmtexp(3:4),'(i2)') 13  
               read(czeile(18:30),cfmtexp) energy
               lflagegy = 1
            endif
         endif
...
c - - - - - - summ all NKG - AVERAGE quantities - - - - - - - - - - - -
  ...
  115    continue
         close(unit=1) 
      enddo
      write(*,'(5x,''lst='',i7,8x,''energy='',1p,e12.4,
     +   '' GeV'',9x,''listnkg'')') lst,energy
      do  il=1,jl 
