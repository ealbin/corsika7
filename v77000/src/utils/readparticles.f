...
c  print tabular of counted particles also for more than one shower
c  in the particle data file;
c-----------------------------------------------------------------------
c CompLink:
...
c but only the first record can be read more or less exactly.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      character cdata(80)*250,cdat*250
...
* - - - - - - check on original 64bit simulation:
     ...
     +                 .and. pdatone .lt. 3.23d-41 ) ) goto 443
      if (mod(irec,4000) .eq. 0) write(*,'(27x,''irec ='',i8)') irec
* - - - - - - - - - - -
      if ( ishift .eq. 1 ) then
...
c=======================================================================
      ...
      double precision aatm(5),batm(5),catm(5)
      double precision parmas(0:101),phead(30)
      character cdata(80)*250,qpatext(1:200)*19
      dimension qpartic(0:400),jpartic(0:201,32)
      real pdata(lenthin)
      common /atmos/aatm,batm,catm
      common /integ/lobs,lsho,irec,isho,isub,idat,ilen,ifil,jpartic
