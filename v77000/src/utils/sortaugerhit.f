...
c RunScript:
c     ./sortaugerhit.sh
c     ./sortaugerhit.sh 2084
c     ./sortaugerhit.sh 672012
c     ./sortaugerhit.sh DAT672012
c     ./sortaugerhit.sh DAT672012.part
c RunProg:
...
c - - - - - - read all names of particle data files (up to nfmx):
  ...
  108 continue
      ...
      if ( lfilen .ne. 9.and. lfilen .ne. 14.and. lfilen .ne. 16 ) then
...
c - - - - - - read first record of first file to test simulation:
     ...
     +     form='unformatted',access='sequential')
      ...
      if ( lfilen .eq. 9 .or. lfilen .eq. 14 ) cfilename(1)(10:10) = '-'
c - - - - - - detect `standard` simulation instead of `thinning`:
     ...
     +   pdata(48+lenblk)*1.d-2,20000000+nint(pdata(3)),pdata(4)
      ...
      do  i=937,1560 ! init for evte and rune subblocks.
         qdata(i) = 0.
      enddo
      write(*,'(/,12x,a)') cfilename(1)(1:lfilen)
...
c - - - - - - test/copy augerhit parameters and calculate file names:
     ...
     +   abs(qdata(lenblk+7)*1.d-2)
      ! qdata(lenblk+98) = 0. ! opt. calculate new set.
...
c - - - - - - calculate new positions of scattered cores:
      ...
      if ( qdata(lenblk+98) .lt. 0.d0 ) then
        if ( first ) then
          ...
          first = .false.
          call sobseq( -2,rnd )
          do  i=1,irun+irun ! iseed(1,3) or irunnr
            call sobseq( 2,rnd )
          enddo
        endif
        do  lcor=1,ncores
...
c - - - - - - sort particles to matching core positions: 
     ...
     +   f8.2,'' m   y='',f8.2,'' m   r='',f8.2,'' m  __'')')
     +   lcor,xshc,yshc,sqrt(xshc*xshc+yshc*yshc)
      write(*,'(14x,a)') cfilenout(lcor)(1:28)
      muaddi = 0
      mzeros = 0
      mtoobig= 0
...
c - - - - - - loop on names of particle data files:
      ...
      iend = 0
      irec = 0
      open(unit=1,file=cfilename(ifil)(1:lfilen),form='unformatted',
...
c - - - - - - end-of corsika particle data file reached - - - -
     ...
     +         f10.7,'')'',i5,'' records'')') tpoi,tsum,tpoi/tsum,
     +         int((tpoi+818.d0)/819.d0)
         else
            if ( tpoi .ge. 1.d0 ) 
     +      write(*,'(10x,f14.0,'' particles of'',f14.0,'' ('',
     +         f10.7,'')'',i5,'' record '')') tpoi,tsum,tpoi/tsum,
     +         int((tpoi+818.d0)/819.d0)
         endif
