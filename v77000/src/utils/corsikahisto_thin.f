C=======================================================================
C
C  c o r s i k a h i s t o _ t h i n . f  with "thinning"
C           ====================================================
...
C----------------------------------------------------------------------
C     compilation:
C         gfortran -fbounds-check -frecord-marker=4 corsikahisto_thin.f -o corsikahisto_thin
...
C----------------------------------------------------------------------
C     J.Oehlschlaeger, D. Heck, T. Pierog 05 October 2015 
C=======================================================================
...
C=======================================================================
      ...
      CHARACTER CHV(6552)*4,CIDENT*4,CDAT*70
     ...
     &           ,i1mom(0:maxmm),i2mom(0:maxmm),i3mom(0:maxmm)
      integer maxiz,numiz
      integer ngenmx
      parameter (ngenmx=1000)
      double precision cntgen(0:ngenmx,0:9,2)
...
C----------------subblock run header------------------------------------
             PDATA(LIA) = 11111111.
c             DO    IL=LIA,LIA+311,8
c                WRITE(7,'(1P,8E13.5)') (PDATA(II+IL),II=0,6)
c             ENDDO
...
C-----------subblock with particle data---------------------------------
           DO    IL=LIA,LIA+311,8
c             WRITE(*,'(1P,8E16.8)') (PDATA(II+IL),II=0,7)
             idc=nint(PDATA(IL))
             if(idc.gt.0)then
               id=idc/1000
               if(id.lt.70)then
...
c                     if(gen.gt.50d0)then
c                       gen=gen-50d0
c                     elseif(gen.gt.30d0)then
c                       gen=gen-30d0
                 if(ihistory.eq.2)then
                   iimode=2                 !history
                   gen=gen+genem
           call cana(h0,x0,y0,t0,E0,px,py,pz,am,wt,gen,id0,idi,iimode)
                   ihistory=0     !reset after grandmother
                 endif
c           if(mod(gen,1000d0).gt.100)print *,gen,idc,id1,id0,gen0,gen1
                 lvl=mod(idc,10)
                 h1=obslvl(lvl)
...
c     .           tfront(lvl)=0.25d0*(t1-tfront(lvl))
                 ...
                 gen=dble(mod(idc,1000))
                 iimode=10       !additional info
                 t1=-1d0
                 px=PDATA(IL+1) !px momentum GeV/c
                 py=PDATA(IL+2) !py momentum GeV/c
                 pz=PDATA(IL+3) !pz momentum GeV/c
                 x1=PDATA(IL+4)/100d0 !x in m
                 y1=PDATA(IL+5)/100d0 !y in m
                 h1=PDATA(IL+6)/100d0 !height in m
                 wt=0d0         !weight
                 idi=idtrafo('cor','nxs',id)
                 call idmass(idi,am)
                 E1=sqrt(px*px+py*py+pz*pz+am*am)
               endif
               call cana(h1,x1,y1,t1,E1,px,py,pz,am,wt,gen,idi,0,iimode)
             elseif(idc.lt.0)then
               ihistory=ihistory+1
               id=abs(idc)/1000
c               print *,"history",ihistory,id
               if(ihistory.eq.1)then    !mother
                 iimode=1       !history
                 gen1=dble(mod(abs(idc),1000))
                 t1=-1d0
                 px=PDATA(IL+1) !px momentum GeV/c
                 py=PDATA(IL+2) !py momentum GeV/c
                 pz=PDATA(IL+3) !pz momentum GeV/c
                 ...
                 h1=PDATA(IL+6)/100d0 !height in m
                 wt=0d0         !weight
                 id1=idtrafo('cor','nxs',id)
                 call idmass(idi,am)
                 E1=sqrt(px*px+py*py+pz*pz+am*am)
             call cana(h1,x1,y1,t1,E1,px,py,pz,am,wt,gen,id1,0,iimode)
               elseif(ihistory.eq.2)then      !grandmother
                 gen0=dble(mod(abs(idc),1000))
                 px=PDATA(IL+1) !px momentum GeV/c
                 py=PDATA(IL+2) !py momentum GeV/c
                 pz=PDATA(IL+3) !pz momentum GeV/c
                 genem=PDATA(IL+4)*100D0!generation
                 z0=PDATA(IL+5) !chi in g/cm2
                 h0=abs(PDATA(IL+6))/100d0 !height in m
                 wt0=PDATA(IL+7) !weight
                 id0=idtrafo('cor','nxs',id)
                 call idmass(id0,am)
                 E0=sqrt(px*px+py*py+pz*pz+am*am)
                 x0=0d0
                 y0=0d0
               endif
             else
...
c     wt                 =  weight
c     gen                =  generation
c     iimode             =  particle at gound (0) or additional info (10) or mother (1) or grandmother (2)
c
...
c        added----------vc300304---from cana
      ...
      igen0=mod(int(gen),1000)
c      print *,gen,igen,iimode
      if(igen0.gt.0.and.igen0.le.ngenmx)then
        cntgen(0,0,k)=cntgen(0,0,k)+wt
        j=9
        igen=mod(int(gen/10**j),10)
        cntgen(igen,j,k)=cntgen(igen,j,k)+wt
        j=8
        igen=mod(int(gen/10**j),10)
        cntgen(igen,j,k)=cntgen(igen,j,k)+wt
        j=6
        igen=mod(int(gen/10**j),100)
        cntgen(igen,j,k)=cntgen(igen,j,k)+wt
        igenem=igen
        j=3
        igen=mod(int(gen/10**j),1000)
        cntgen(igen,j,k)=cntgen(igen,j,k)+wt
        igenem=igenem+igen
        if(igenem.eq.0)cntgen(igen0,0,k)=cntgen(igen0,0,k)+wt
      endif
      ...
      it=min(max(1,int(log(tt/tamin)/log(ctime)+1.5d0)),maxie)
...
c Generation Profile **********************************************
     ...
     &           ,' Generation profile  ************************'
      ...
      do k=1,2
        if(cntgen(0,0,k).gt.0d0)then
          genmn=0d0
          genmx=0d0
          do i=1,ngenmx
            do j=0,9
              cntgen(i,j,k)=cntgen(i,j,k)/ cntgen(0,0,k)
              if(cntgen(i,j,k).gt.0d0)genmn=max(genmn,dble(j))
            enddo
            if(cntgen(i,0,k).gt.0d0)genmx=max(genmx,dble(i))
          enddo

          if(k.eq.1)then
            write(ifho,'(a)')  'openhisto name GenerationEM '
            write(ifho,'(a)')  'htyp prc'
          endif
          if(k.eq.2)then
            write(ifho,'(a)')  'openhisto name GenerationHad'
            write(ifho,'(a)')  'htyp pbs'
          endif
          if(genmn.gt.0d0)then
            jmx=int(genmn+1d0)
            genmx=10d0**jmx
            write(ifho,'(a)')  'xmod log ymod log'
            write(ifho,*)      'xrange ',0.5d0,genmx
          else
            jmx=0
            write(ifho,'(a)')  'xmod lin ymod lin'
            write(ifho,*)      'xrange ',0d0,genmx
          endif
...
c end column names
          ...
          jx=0
          do ix=1,ngenmx-1
            write(ifho,'(i10,1p,e13.5)')ix,cntgen(ix,jx,k)
          enddo
          jx=3
          do ix=1,ngenmx-1
            write(ifho,'(1p,e10.3,e13.5)')ix*10d0**jx,cntgen(ix,jx,k)
          enddo
          jx=6
          do ix=1,99
            write(ifho,'(1p,e10.3,e13.5)')ix*10d0**jx,cntgen(ix,jx,k)
