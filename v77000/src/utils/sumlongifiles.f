...
c  ls -1 DAT*.long > sumlongifiles.i002543
c  ./sumlongifiles < sumlongifiles.i002543 > sumlongifiles.out002543
c  cp -p sumlongifiles.sum002543 DAT002543-999989999.long
c-----------------------------------------------------------------------
...
c - - - - - - write sum tables - - - - - - - - - - - - - - - - - - - - -
      open(unit=44,file=chlongsum,form='formatted',
     +     access='sequential',status='unknown')
      write(44,'(a)') czeilong(1)
      write(44,'(a)') czeilong(2)
      do  is=1,lsteps
         write(44,'(f6.0,1p,9e12.5)')
     +      qxdistr(is),(qdistrb(il,is),il=1,9)
      enddo
      write(44,'(a)') czeilong(3)
      write(44,'(a)') czeilong(4)
      engysum = 0.d0
      do  is=1,lsteps
         engysum = engysum + qenergy(9,is) 
         write(44,'(f6.1,1p,9e12.5)')
     +      qxenerg(is),(qenergy(il,is),il=1,9)
      enddo
      close(unit=44)
