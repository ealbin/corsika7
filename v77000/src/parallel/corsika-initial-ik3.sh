...
#       using /cr/auger02/joe/corsika.trunk/run/ as main path.
# Usage:
...
if [ $# -lt 2 ] ; then
   ...
   echo "./preprocess-ik3.sh parallel-002413 corsika74088_stnd_QGSII4_gheisha_pll"
   echo "In path csk002413/ start the parallel corsika simulation by typing:"
   echo "./corsika-initial-ik3.sh parallel-002413 corsika74088_stnd_QGSII4_gheisha_pll ik3"
...
########## correct number of arguments (2 or 3) ##########
   ...
   case $TYPE in
       ...
       "ik3")      
                ...
                qsub -l q=augx.q corsika-initial.ik3.sh;;
       "LL")      
                echo " no longer available"
                numrunning=`expr $numrunning + 1`;;
                # llsubmit corsika-initial.ll.sh;;
       "BOINC")    
                echo "not at all available"
                exit 2;;
       "GRID")   
                echo "not available, not tested"
