...
void read_input(double* dectcut,double* dectmax,int* lprim,char* cutfilename,int* id1,int* id2,int* rnnum,int* seed,char * input_fname)
{
			...
			//if(seeder) 
				...
				*dectmax= strtod(str3,&end);
				...
				if(!strcmp(str5,"T")||!strcmp(str5,"t")) lcout= 1;
						...
						{
            // Tim Huege: check if path has quotes and remove them if this is the case, assumes first quote as first character of str2
            if ((str2[0]=='\'')||(str2[0]=='\"'))
            {
              char strtmp[180]; // same hard-coded length as for str2 above
              int i;
              for (i=1; i<179; ++i)
              {
                strtmp[i-1]= str2[i];
                if ((str2[i+1]=='\'')||(str2[i+1]=='\"')||(str2[i+1]=='\0'))
                {
                  strtmp[i]='\0';
                  break;
                }
 ...
 if (dbg_infos) 
	{
	printf("Successfully read the input file %s\n",input_fname);
	fflush(stdout);
	}
 fclose(file);
}
...
/*
function printstatusstop()
Called from CORSIKA, for debugging purpose
*/

void printstatusstop_(int* rnnum,int* uid,char cftext[255])
{
 int rank;
 char statfile[255],numstr[9];
 FILE *file;
 MPI_Comm_rank (MPI_COMM_WORLD, &rank);
	...
	strcat(statfile,"-runprotocol.txt");
	file = fopen(statfile,"a"); 
	fprintf(file,"\n\n\n[%f] CORSIKA: Shower from simulation under run number %d stopped :%s \n\n",MPI_Wtime(),*rnnum,cftext);
	fclose(file); 
