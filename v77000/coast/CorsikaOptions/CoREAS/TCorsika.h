...
class TCorsika {
 ...
 public:
  ...
  //~ double GetDistanceOfPoint(const double x, const double y, const double z) const;  // this was previously used in SetFirstInteraction() to set fFirstInteractionDist, which then is actually not used anywhere

  //~ void DumpAtmosphereTable() const;
  
  //~ double GetLastLevelSlantDepth() const;

  //~ double GetMaximumTabulatedDepth() const;
