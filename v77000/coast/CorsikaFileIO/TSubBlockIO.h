/**
   ...
   \version $Id: TSubBlockIO.h 5116 2016-01-04 19:09:04Z darko $ 
...
namespace crsRead {
  ...
  */
  ...
  public:
    ...
    TSubBlockIO(bool Thinned, unsigned int NBlockSizeInfo=sizeof(float));
    ~TSubBlockIO(); 
    
    void InitSubBlockSize(bool thinn, unsigned int NBlockSizeInfo=sizeof(float));
  ...
  private:
    ...
    void InitBuffer();
    void InitBlockSizeInfo();
    
  protected:
    int fBlockSize;            ///< Actual size of one CORSIKA SubBlock
    unsigned int fNBlockSizeInfo;       ///< Number of Additional bytes after 
    ///                                each Block(21 SubBlocks). (-> one Fortran Record)
