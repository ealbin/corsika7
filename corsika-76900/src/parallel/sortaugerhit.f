...
c   ---------------------------
c     sort reduced particle data output (by keyword AUGSCT) by writing
c     particles to separate particle additional data files each of a
c     single matching core position.
c-----------------------------------------------------------------------
c compilation:
...
c     # mv fort.20 DAT000070.prtcls20
c out-files:
...
c-----------------------------------------------------------------------
      ...
      double precision xcore(20), ycore(20), rnd(2)
...
c - - - - - - read all names of particle data files (up to nfmx):
  ...
  104 continue
      ...
      goto 104
  107 continue
      write(*,*) '    ERROR: reading list of particle data files.'
  108 continue
      if ( lfil .ne. 9 .and. lfil .ne. 16 ) 
     +   write(*,*) '    WARNING: check name of particle data file!'
      nfiles = ifil - 1
...
c - - - - - - read first record of first file to test simulation:
     ...
     +     form='unformatted',access='sequential')
      ...
      close(unit=1)
      if ( lfil .eq. 9 ) cfilename(1)(10:10) = '-'
c - - - - - - detect `standard` simulation instead of `thinning`:
     ...
     +                       pdata(312+1) .lt. 217433.2 ) then
      ...
      lenblk = lenrec / 21
      lenpar = lenblk / 39
      write(*,'(/,6x,''runnr ='',i7.6,6x,''log10(energy) ='',f8.4,
     +   6x,''theta(deg) ='',f6.2)') irun,
     +   log10(1.d9*pdata(lenblk+4)),57.29578*pdata(lenblk+11)
      ...
      if ( qdata(274)+qdata(547) .lt. 1. ) then
       write(*,'(/,12x,''corsika particle data from `thinning run`'')')
      else
       write(*,'(/,12x,''corsika particle data from `standard run`'')')
      endif
      write(*,'(12x,''number of particle files ='',i7)') nfiles
c - - - - - - test/copy augerhit parameters and calculate file names:
      if ( qdata(lenblk+98) .le. 1.d0 .or. qdata(lenblk+176) .le. 1. )
     + then ! number of scattered cores originally not set.
         write(*,'(12x,''WARNING: no core positions defined;'',
     +      '' calculate new ones:'')')
         ncores = 20
...
c - - - - - - calculate new positions of scattered cores:
      if ( qdata(lenblk+98) .le. 0.d0 ) then
        write(*,'(2x)')
        if ( first ) then
...
c - - - - - - sort particles to matching core positions: 
      ...
      xshc = xcore(lcor)
      yshc = ycore(lcor)
      qdata(lenblk+176) = lcor ! current number of core position.
...
c - - - - - - end-of corsika particle data file reached - - - -
     ...
     +      '' records'')') tpoi,tsum,int((tpoi+800.d0)/819.d0)
         tpoisum = tpoisum + tpoi
         trecsum = trecsum + int((tpoi+800.d0)/819.d0)
      endif
  444 continue
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
c - - - - - - - - switch particle coordinates to meter:
             pdata(lp+4) = 1.d-2 * pdata(lp+4)
             pdata(lp+5) = 1.d-2 * pdata(lp+5) 
...
c - - - - - - test coordinates on 60 degrees stripes:
     ...
     +          goto 944
             ...
             if ( lcor .eq. 20
     +          .and. ( iytst .eq. 0 .and. ixtst .eq. 0 ) ) then
                ! test print of only lcor=20 selected particles:
                write(20,'(i5,''.'',1p,3e12.4,0p,2f10.2,f14.8,f8.2,
     +             2f9.2)') icode,(pdata(i),i=lp+1,lp+lenpar-2)
     +             ,pdata(lp+lenpar-1)*1.d-6,sqrt(pdist)
...
c - - - - - - - - - - particle belongs to lcor-th core position:  
     ...
     +               ''pz'',9x,''x[cm]'',7x,''y[cm]'',7x,''t[nsec]'',
     +                2x,''weight'',2x,''dist[m]'')')
                  else
                    write(*,'('' icode'',5x,''px'',10x,''py'',10x,
     +               ''pz'',10x,''x[m]'',6x,''y[m]'',7x,''t[nsec]'',
     +                2x,''dist[m]'')')
                  endif
                endif
                ! test print of first lcor-th particles: 
                if ( tpoi .le. 3. ) then
                   if ( pdata(lp+lenpar-1) .ge. 1. )
     +             pdata(lp+lenpar-1) = 1.d-6*pdata(lp+lenpar-1) + lcor
                   write(*,'(i5,''.'',1p,3e12.4,
     +               0p,2f10.2,f14.8,f8.2,2f9.2)') icode,
...
**          if ( int(1.*(lp-lia)/lenpar) .gt. 0 )
**   +         write(*,'(12x,''irec ='',i11,'' last;   particle='',i2.2,
**   +         ''   isub='',i2.2,i10,a)') irec,int(1.*(lp-lia)/lenpar),
**   +         int((1.*lenblk+lia)/lenblk),lsub,'=lsub'
            write(*,*) '        irec =',irec,'   null =',pdata(lp)*1.e-3
            goto 946 
          elseif ( pdata(lp) .ge. 201000. ) then
            if ( lcor .eq. ncores) write(*,*) '        ifil =',ifil,
...
c - - - - - - calculate mcores centers of showers: 
      do  i=1,mcores
         lx = (xcore(i)+10.)*5.0d-2 ! 20 units / char. 
         ly = (ycore(i)+20.)*2.5d-2 ! 40 units / line.
         charray(lx,ly) = '@' 
      enddo
