...
c     the line `head -108` in the script `readtimes.sh` must give the
c     actual line of all simulation protocol files `DATijklmn.lst`   
...
c execution by script (incl. argument):
c     ./readtimes.sh DAT4012
c     # to sum up time info from all files DAT4012??.lst
c-----------------------------------------------------------------------
cDAT401200.lst
...
c - - - - - - read 5 lines (2,3 containing times):
         ...
         read(*,'(a)') czeile(4)
         read(*,'(a)') czeile(5)
         if ( lfiles .eq. 1 ) write(*,'(15x,a)') czeile(1)
...
c - - - - print sum of run times:
      if ( lfiles .gt. 3 ) write(*,'(15x,6(''. ''))')
      if ( lfiles .gt. 1 ) write(*,'(15x,a)') czeile(1)
      write(*,'(f33.2,'' hours ='',f11.4,'' days'')')
     +   hoursum,hoursum/24.
      write(*,'(40x,''='',f11.4,'' days/evt'')') hoursum/24./lfiles
      write(*,'(40x,''='',f11.4,'' hours/evt'')') hoursum/lfiles
