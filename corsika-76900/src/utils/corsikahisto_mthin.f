C=======================================================================
C
C  c o r s i k a h i s t o _ m t h i n . f  (with MULTTHINNING)
C           ====================================================
...
C----------------------------------------------------------------------
C     compilation:
C         gfortran -fbounds-check -frecord-marker=4 corsikahisto_mthin.f -o corsikahisto_mthin
C         f77 -fbounds-check -m32 corsikahisto_mthin.f -o corsikahisto_mthin
C         ifort -C corsikahisto_mthin.f -o corsikahisto_mthin
C----------------------------------------------------------------------
...
C--OPEN output file-----------------------------------------------------
      name=index(cname,' ')
      hname=cname(name-9:name-1)//'.histo '
      write(*,*)"Output file name :",hname
...
C=======================================================================
     ...
     &,iefirst,moden
      ...
      parameter(maxiz=7,maxjz=maxiz,maxir=101,maxjr=3
     ...
     & ,specr(maxin,maxie,maxjz,maxjr),speca(maxin,maxjz,maxjr)
      parameter        (rtank = 1.8)  ! tank radius [m]
      parameter        (htank = 1.2)  ! tank height [m]
      logical pass,hit
...
c             ENDDO
             obslvl(1)=PDATA(LIA+5)/100d0 !height in m
...
c             height0=hatm
             ...
             if(abs(costh).gt.0.0001)then
               tfront(1)=(height0-obslvl(1))/0.299792458D0
             else
               tfront(1)=1d30
             endif
             numiz=nint(PDATA(LIA+176))
             print *,numiz,' weight level'
             do IL=0,numiz-1
               print *,IL+1,' with weight ',PDATA(LIA+183+IL),' below '
     .                     ,PDATA(LIA+177+IL),' for hadrons' 
               print *,IL+1,' with weight ',PDATA(LIA+195+IL),' below '
     .                     ,PDATA(LIA+189+IL),' for EM' 
             enddo
             numiz=numiz+1
             zamin=-1d0
             zamax=dble(numiz)+1d0
             rsamp=PDATA(LIA+173)/100d0
             if(rsamp.gt.0d0)then
             !if Augerhit, rescale thinned particle weight to the surface of tankshadow
               sampl=2*sqrt(3d0)*rsamp**2     !surface of the hexagone
               print *,"augerhit",rsamp,sampl
             else
               sampl=0d0
             endif
...
C-----------subblock with particle data---------------------------------
           DO    IL=LIA,LIA+272,7
             ...
             id=idc/1000
c               print *,idc
             if(id.ne.8888.and.id.gt.0)then
               if(id.lt.70)then
...
c               print *,'norm',id,wt
     ...
     .           call cana(h1,x1,y1,t1,E1,px,py,pz,am,wt,gen,idi,iimode)
             ...
             elseif(id.eq.8888)then
               hit=.true.
               i0=2
               if(rsamp.gt.0)then
                 i0=1
                 hit=mod(idc,1000).ge.100 !tankshadow
                 cospa=pz/sqrt(px*px+py*py+pz*pz)
                 stank=abs(cospa)*3.141592*rtank**2
     .                +sqrt(1d0-cospa**2)*2d0*rtank*htank
                 whit=stank/sampl
               else
                 whit=1d0
               endif
               do ii=i0,numiz
                 ...
                 if(ii.eq.1)then
                   ...
                   wt=PDATA(IL+ii-1)
               !count all to increase static of thinned particles but with rescaled area
                   pass=.true.
                   wt=wt*whit
                 endif
c                 print *,'weight',id,wt
                 if(pass.and.wt.gt.0)
     .           call cana(h1,x1,y1,t1,E1,px,py,pz,am,wt,gen,idi,iimode)
               enddo
             else
...
c     wt                 =  weight
c     gen                =  generation
c     iimode             =  particle at gound (1 or weight index) or additional info (-1)
c
...
c        added----------vc300304---from cana
      ...
      igen=int(gen)
      if(iimode.eq.1.and.igen.le.ngenmx)then
      ...
      if(in.gt.10)yieldz(10,iz)=yieldz(10,iz)+wt
      xx=x1
      yy=y1
      tt=t1-tfront(1)

      it=min(maxie,max(1,int(log(tt/tamin)/log(ctime)+1.5d0)))
      yiex(in,iz,it)=yiex(in,iz,it)+wt
      if(in.gt.10)yiex(10,iz,it)=yiex(10,iz,it)+wt
...
c end column names
     ...
     &                           ,eprima*fev,'"'
      write(ifho,'(a)')  '- txt  "xaxis Thinning"'
      if(kkk.eq.1)then
      ...
      do iz=1,numiz
        ...
        enddo
        if(kkk.ne.3)then
          write(ifho,'(e13.5,90e11.3)')zi,stot(iz)/max(1d0,stot(1))
     & ,(yieldz(ip,iz)/max(1d0,yieldz(ip,1)),ip=k1,k2)
        else
          write(ifho,'(e13.5,90e11.3)')zi,stot(iz)/max(1d0,stot(1))
     & ,(yieldz(ip,iz)/max(1d0,yieldz(ip,1)),ip=k1,k2)
     & ,(yieldz(2,iz)+yieldz(3,iz))/(yieldz(2,1)+yieldz(3,1))
        endif
      end do
      write(ifho,'(a)')       '  endarray'
      write(ifho,'(a)')       'closehisto'
