c=======================================================================
c
c  r e a d m t h i n m u o n s . f
c  ------------------------------- 
...
c-----------------------------------------------------------------------
c compile+link:
c     gfortran -O0 -fbounds-check readmthinmuons.f -o readmthinmuons 
c     ifort -C -O0 -check bounds readmthinmuons.f -o readmthinmuons
c-----------------------------------------------------------------------
c select-files:
c   echo "DAT151522" > readmthinmuons.inpmthi
c   ls -1 DAT111888 > readmthinmuons.i111888
c   ls -1 DAT001515-* | grep t -v | grep n -v > readmthinmuons.inpmthi
c   ls -1 DAT001522-0????? > readmthinmuons.inpmthi
c running:
c   ./readmthinmuons < readmthinmuons.inpmthi
c   ./readmthinmuons < readmthinmuons.i111888 > readmthinmuons.out111888
c-----------------------------------------------------------------------
c input-files:
c           unit=*: name(s) of particle data file(s).
c           unit=3: opened current corsika particle data file.
c output-file: 
c           unit=*: protocol output;
c           unit=8: muon particle text tabular file DAT....muons;
c-----------------------------------------------------------------------
...
c--read all names of particle data files (one shower)-------------------
      write(*,'(1x,6(''_ ''),''readmthinmuons.f'',24('' _''))')
      do  ifil=1,25000
...
c--check particle data subblocks on multi-thinning identification:
      ...
      do  l=1+2*lenblk,5*lenblk,lenpar
         icode = int(1.d-3*(pdata(l)*1.0000001d0))
...
c--get total number of multi-thinning levels:
      ...
      if ( mthins .gt. 0 ) then
        write(*,'(12x,''mlevel'',16x,''maxweight'',15x,''maxweight'')')
        do  mlv=1,mthins
...
* - - - - - - read data record with lenrec words - - - -
  ...
  432 continue
* - - - - - - end of corsika particle data files reached: 
     ...
     +      (mlv,mlv=0,mthins)
          do  l=1,46
            if ( qparsum(l,0) .gt. 0. ) write(*,'(i8,7f18.1)')
     +         l,qpartic(l,0),(qparmth(l,mlv),mlv=1,mthins)
          enddo
        endif
      endif
  444 continue
...
c--end of current thinning level reached--------------------------------
     ...
     +    ''weight >= 1'')')
        ...
        write(*,'(2x)')
        write(*,'(6x,''ratios of particles with at least one '',
...
c--print total counts of particles--------------------------------------
     ...
     +        l,qpatext(l),qpartic(l,0)
      ...
      if ( lenblk .eq. 312 ) then
         write(*,'(6x,''---- ----'',12x,''--------------- '')')
         do  l=1,46
            if ( qpartic(l+100,0) .gt. 0. ) write(*,'(i10,a19,f14.1)')
     +           l+100,qpatext(l),qpartic(l+100,0)
         enddo
...
c--print total counts of innermost circle at tank positions-------------
     ...
     +    9x,''(3m/all)'')')
        ...
        enddo
        if ( mthins. eq. 0 ) then ! and also mtype8 = 0.
          write(*,'(2x)')
          write(*,'(38('' _''))')
          write(*,'(/,6x,''code name'',12x,''total particles '',a)')
     +      cdata(nfil)(idat:ilen)
...
c----------------subblock event header----------------------------------
         ...
         phead(2) = 1.d-5 * pdata(lia+6) ! first interaction (km).
         pdata(lia+4) = real(thickgr(dble(pdata(lia+6)))) ! (grams/cm^2)
         phead(3) = pdata(lia+4)
...
c - - - - - - icode = 8888: count all particles with any weight:
           ...
           if ( l .gt. lenpar ) then
             ...
             if ( jcode .eq. 5 .or. jcode .eq. 6 ) then 
               ! write(8,'(1p,8e14.6)') (pdata(l-lenpar+i),i=0,lenpar-1)
               write(8,'(1p,8e14.6)') (pdata(l+i),i=0,lenpar-1)
             endif
           else if ( l .eq. 1 ) then
             ...
             if ( jcode .eq. 5 .or. jcode .eq. 6 ) then 
               ! write(8,'(1p,8e14.6)') (phead(i),i=30,30+lenpar-1)
               write(8,'(1p,8e14.6)') (pdata(l+i),i=0,lenpar-1)
             endif
           endif
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
           if ( jcode .gt. 50 ) then
              if ( jcode .eq. 75 ) then
                 ...
                 jcode = 4 ! count `exotic` nuclei.
              else
                 if ( jcode .ge. 85 .and. jcode .le. 96 ) then
                    jcode = 47 ! ignore 85,86,95,96.
                 endif
                 if ( jcode .ge. 8888 ) jcode = 50
              endif
           endif
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
           if ( mthins .gt. 0 ) then
             do  mlv=1,mthins
               indeprec(mlv) = 0 ! only index mlv=1 is used of this array.
             enddo
